"use strict";exports.id=757,exports.ids=[757],exports.modules={29574:(e,s,t)=>{t.d(s,{BW:()=>r,S7:()=>a,j8:()=>n}),t(15424);var o=t(46242),a=(0,o.$)("55c1a9a14f85a9a0b41568489175e25f84ca3044"),r=(0,o.$)("ddc9f5b14f2aa100e488a7cda2727190bebb6350"),n=(0,o.$)("bd26dc79d397f9193f3c356a47ab87f57072d3ef")},78181:(e,s,t)=>{t.d(s,{Z:()=>i});var o=t(10326),a=t(77626),r=t.n(a),n=t(17577);let i=({name:e,options:s,label:t,onChange:a,isRecommend:i})=>{let[p,c]=(0,n.useState)("");return(0,o.jsxs)("div",{className:"jsx-1272e39a03fc3df6 dropdown",children:[t&&o.jsx("label",{className:"jsx-1272e39a03fc3df6",children:t}),(0,o.jsxs)("select",{required:!0,name:e,value:p,onChange:e=>{let s=e.target.value;c(s),a&&a(s)},className:"jsx-1272e39a03fc3df6 w-full text-black px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500",children:[o.jsx("option",{value:"",disabled:!0,className:"jsx-1272e39a03fc3df6",children:"---"}),s.map((e,s)=>o.jsx("option",{value:i?JSON.stringify(e):e.value,className:"jsx-1272e39a03fc3df6",children:e.value},s))]}),o.jsx(r(),{id:"1272e39a03fc3df6",children:".dropdown.jsx-1272e39a03fc3df6{margin-bottom:16px}label.jsx-1272e39a03fc3df6{margin-right:8px}select.jsx-1272e39a03fc3df6{padding:8px;font-size:16px}"})]})}},1223:(e,s,t)=>{t.r(s),t.d(s,{InvokeOpenAI:()=>O,makeServerCall:()=>N,recommendNewTitle:()=>K});var o=t(27745);t(26461);var a=t(57147),r=t(71017),n=t.n(r),i=t(67590);let p=async(e,s,t)=>{let o=await (0,i.getDirectoryName)(t);await (0,i.checkIfExists)(n().join(`${process.cwd()}/public`,"blog-posts"))||await a.promises.mkdir(n().join(`${process.cwd()}/public`,"blog-posts")),await (0,i.checkIfExists)(n().join(`${process.cwd()}/public/blog-posts`,s))||await a.promises.mkdir(n().join(`${process.cwd()}/public/blog-posts`,s)),await (0,i.checkIfExists)(n().join(`${process.cwd()}/public/blog-posts/${s}`,o))||await a.promises.mkdir(n().join(`${process.cwd()}/public/blog-posts/${s}`,o)),await (0,i.checkIfExists)(n().join(`${process.cwd()}/public/blog-posts/${s}/${o}`,"images"))||await a.promises.mkdir(n().join(`${process.cwd()}/public/blog-posts/${s}/${o}`,"images"));let r=`${process.cwd()}/public/blog-posts/${s}/${o}/index.md`;try{console.log("now writting File"),await a.promises.writeFile(r,e),console.log("File written successfully")}catch(e){console.error("Error:",e)}};var c=t(74739);let l=async(e,s)=>`Act as a technical content writer. Before creating content, make sure to include a link to the product name whenever it appears in the content. Additionally, use plenty of transition words to improve the flow. Please replace any naked links with relevant words or phrases as anchor text whenever they appear in the content. The headings should use "##" without any numbering to keep the structure clean and straightforward.
...
`,m=async(e,s)=>`I have the following list of existing article titles focused on ${s.value} in various languages. 

    ${e.join("\n")} 

    Now, based on the features officially supported by ${s.value} (see docs: ${s.DocumentationURL} and API ref: ${s.APIReferenceURL}, please generate 5 new, unique article titles that:

    Do not reuse or overlap with any of the existing titles above

    Are strictly based on what ${s.value} can do (no unsupported suggestions)
...
`,d=async(e,s)=>{let t=e.get("title")?.toString().replace(/\bonline\b/gi,"").replace(/\s{2,}/g," ").trim();return`Act as a technical content writer. Before creating content, make sure to include a link to the product name whenever it appears in the content. Additionally, use plenty of transition words to improve the flow. Please replace any naked links with relevant words or phrases as anchor text whenever they appear in the content. The headings should use "##" without any numbering to keep the structure clean and straightforward. 
    ...
    ${e.get("additionalInstructions")}
    `},u=async(e,s,t)=>{let o=await (0,i.normalizeHeadings)(t.toString()),a=await (0,i.parseInputToObjects)(o);console.log("here is the output -- start"),console.log(a),console.log("here is the output  -- end");let r=await (0,i.titleCase)(e.get("title")),n=await (0,i.filterSections)(a,"Meta Title"),p=await (0,i.filterSections)(a,"Meta Description"),c=await (0,i.filterSections)(a,"Short Summary"),l=await (0,i.makeBlogURL)(e.get("title"),s.urlPrefix),m=await (0,i.filterSections)(a,"Overview"),d=await (0,i.filterSections)(a,"Library Installation"),u=await (0,i.filterSections)(a,"Code Snippet with a Step-by-Step Guide"),g=await (0,i.filterSections)(a,"Conclusion"),h=await (0,i.filterSections)(a,"Get a Free License"),f=await (0,i.filterSections)(a,"Try Online"),y=await (0,i.filterSections)(a,"Public Resources"),w=await (0,i.filterSections)(a,"Explore"),L=await (0,i.filterSections)(a,"Frequently Asked Questions – FAQs"),R=await (0,i.getDate)(),U=await e.get("secondaryKeywords").split(",").map(e=>e.trim());return`---
title: ${r}
seoTitle: ${n.content}
...
{{< figure align=center src="images/${await (0,i.insertDashes)(r)+".png"}" alt="${r}" >}}
...
${w?.content}
`},g=async(e,s,t)=>{let o=await (0,i.normalizeHeadings)(t.toString()),a=await (0,i.parseInputToObjects)(o),r=await (0,i.titleCase)(e.get("title")),n=await (0,i.filterSections)(a,"Meta Title"),p=await (0,i.filterSections)(a,"Meta Description"),c=await (0,i.filterSections)(a,"Short Summary"),l=await (0,i.makeBlogURL)(e.get("title"),s.urlPrefix),m=await (0,i.filterSections)(a,"Overview"),d=await (0,i.filterSections)(a,"Prominent Features"),u=await (0,i.filterSections)(a,"Usage with a Step-by-Step Guide"),g=await (0,i.filterSections)(a,"How to Implement Programmatically"),h=await (0,i.filterSections)(a,"Conclusion"),f=await (0,i.filterSections)(a,"Get a Free License");await (0,i.filterSections)(a,"Try Online");let y=await (0,i.filterSections)(a,"Public Resources"),w=await (0,i.filterSections)(a,"Explore"),L=await (0,i.filterSections)(a,"Frequently Asked Questions – FAQs"),R=await (0,i.getDate)(),U=await e.get("secondaryKeywords").split(",").map(e=>e.trim());return`---
title: ${r}
seoTitle: ${n.content}
description: ${p.content}
date: ${R}
...
- [Library Installation](#${await (0,i.insertDashes)("Library Installation")})
- [Code Snippet with a Step-by-Step Guide](#${await (0,i.insertDashes)("Code Snippet with a Step-by-Step Guide")}) ${e.get("onlineTool")?`
- [Try Online](#${await (0,i.insertDashes)("Try Online")})`:""}
- [Useful Resources](#${await (0,i.insertDashes)("Useful Resources")})

## Library Installation {#${await (0,i.insertDashes)("Library Installation")}}

${d.content}

## Code Snippet with a Step-by-Step Guide {#${await (0,i.insertDashes)("Code Snippet with a Step-by-Step Guide")}}
...
- [Prominent Feature](#${await (0,i.insertDashes)("Prominent Feature")})
- [Usage with a Step-by-Step Guide](#${await (0,i.insertDashes)("Usage with a Step-by-Step Guide")})
- [How to Implement Programmatically](#${await (0,i.insertDashes)("How to Implement Programmatically")})
- [Useful Resources](#${await (0,i.insertDashes)("Useful Resources")})

## Prominent Features {#${await (0,i.insertDashes)("Prominent Feature")}}
...
## Public Resources {#${await (0,i.insertDashes)("Useful Resources")}}
...
Read More: ${s}`);return{success:!0,platform:"x",data:o}}catch(e){return console.error("Error posting to X:",e),{success:!1,platform:"x",error:e}}}(0,P.h)([k]),(0,o.j)("1819a0099bcf0da4ca758299347fb76a9611907e",k);var j=t(86503),T=t(39113);async function B(e){try{let{data:s}=await j.Z.get(e,{headers:{"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64)","Accept-Language":"en-US,en;q=0.9"}});return T.zD(s)("h1").first().text()}catch(e){return console.error("Error scraping:",e.message),null}}async function J(e){let s=[],t=e;try{for(;t;){console.log(`Fetching: ${t}`);let{data:e}=await j.Z.get(t,{headers:{"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64)","Accept-Language":"en-US,en;q=0.9"}}),o=T.zD(e);o("article.post-entry header.entry-header h2").each((e,t)=>{let a=o(t).text().trim();a&&s.push(a)});let a=o("nav.pagination a.next").attr("href");t=a?a.startsWith("http")?a:`https://blog.aspose.com${a}`:null}return console.log(`✅ Fetched ${s.length} titles in total.`),s}catch(e){console.error("Error scraping:",e.message)}}async function O(e){let s;console.log("****************************** form data start"),console.log(e),console.log("****************************** form data end");let t="";"programmatic"==e.get("BlogType")?(s=y.find(s=>s.value===e.get("products")),t=await l(e,s)):(s=w.find(s=>s.value===e.get("products")),t=await d(e,s));let o="",a="";switch("DeepSeek R1"==e.get("model")&&(a=await $(t)),a="Mixtral"==e.get("model")?await b(t):await (0,L.A)({model:e.get("model"),maxTokens:Number(e.get("tokenLimit")),temperature:Number(e.get("temprature")),maxRetries:5,prompt:t}),console.log("here is the raw output -- start"),console.log(a),console.log("here is the raw output  -- end"),e.get("format")){case"Free flowing":o="programmatic"==e.get("BlogType")?await u(e,s,a):await g(e,s,a);break;case"Table of content(TOC)-based":o="programmatic"==e.get("BlogType")?await h(e,s,a):await f(e,s,a);break;default:o=await u(e,s,a)}await p(o,e.get("uuid"),e.get("title")),console.log("all done"),(0,c.redirect)("/download")}async function N(e){if(console.log("****************************** server call form data start hooooo1"),JSON.parse(e.get("platforms")).value&&'""'!==JSON.parse(e.get("platforms")).value)return null;let s=JSON.parse(e.get("platforms")),t=[];for(let o=0;o<s.length;o++)try{let a=await (0,L.A)({model:"gpt-4o",maxTokens:500,temperature:.3,maxRetries:5,prompt:`I want to create a ${s[o].value} post. Please follow these instructions:
          ...
          6. Do not include any introductory or explanatory text. Return only the post content.`});t.push({value:s[o].value,data:a.content||a.text||a||""}),console.log("reslut is ")}catch(e){console.error(`Failed to generate post for ${s[o]}:`,e)}let o=await B(e.get("url")),a=[];return s.find(e=>"linkedin"===e.value)&&a.push(await D(t.find(e=>"linkedin"===e.value),e.get("url"),o)),s.find(e=>"facebook"===e.value)&&a.push(await S(t.find(e=>"facebook"===e.value),e.get("url"))),s.find(e=>"x"===e.value)&&a.push(await k(t.find(e=>"x"===e.value),e.get("url"))),a}async function K(e){let s=JSON.parse(e.get("product")),t=await J(s.BlogsURL),o=await m(t,s);console.log("prompt returned");try{let e=(await (0,L.A)({model:"gpt-4o-mini",maxTokens:1e3,temperature:.3,maxRetries:5,prompt:o})).split("\n").map(e=>e.replace(/^\d+\.\s*/,"").trim()).filter(e=>e.length>0);return{success:!0,list:e}}catch(e){return{success:!1,err:e}}}(0,P.h)([B,J]),(0,o.j)("3ec9aecef918b0c592d09df76e57c8975d0f31d2",B),(0,o.j)("2d69632b14014ffbf4130fd005242514dc5fb8da",J),(0,P.h)([O,N,K]),(0,o.j)("ddc9f5b14f2aa100e488a7cda2727190bebb6350",O),(0,o.j)("bd26dc79d397f9193f3c356a47ab87f57072d3ef",N),(0,o.j)("55c1a9a14f85a9a0b41568489175e25f84ca3044",K)}};